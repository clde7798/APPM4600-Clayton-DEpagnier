...
def driver():
    ...
    t = time.time()
    for j in range(20):
      [xstar,ier,its] = finiteDiffNewton(x0, tol,Nmax)     
    elapsed = time.time()-t
    print(xstar)
    print('inter','Finite Diff: the error message reads:',ier)
    print('real','Finite Diff: took this many seconds:',elapsed/20)
    print('inter','Finite Diff: number of iterations is:',its)
...
def evalJfinite(x, h): 
    fx= (evalF([x[0]+h, x[1]])[0] - evalF(x)[0]) / h
    fy = (evalF([x[0], x[1]+h])[0] - evalF(x)[0]) / h
    gx = (evalF([x[0]+h, x[1]])[1] - evalF(x)[1]) / h
    gy = (evalF([x[0], x[1]+h])[1] - evalF(x)[1]) / h
    J = np.array([[fx, fy], 
        [gx, gy]])
    return J
...
def finiteDiffNewton(x0,tol,Nmax):

    ''' inputs: x0 = initial guess, tol = tolerance, Nmax = max its'''
    ''' Outputs: xstar= approx root, ier = error message, its = num its'''

    for its in range(Nmax):
       J = evalJfinite(x0, np.linalg.norm(x0)*10**-7)
       Jinv = inv(J)
       F = evalF(x0)
